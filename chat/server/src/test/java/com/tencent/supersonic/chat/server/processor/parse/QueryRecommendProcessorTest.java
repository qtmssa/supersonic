...
class QueryRecommendProcessorTest {
    ...
    void processShouldStillPersistGeneratedSuggestionsWhenRecallFails() throws Exception {
        ...
        try (MockedStatic<ContextUtils> mockedContext = Mockito.mockStatic(ContextUtils.class)) {
            ...
            invokeDoProcess(processor, parseContext);
    ...
    private void invokeDoProcess(QueryRecommendProcessor processor, ParseContext parseContext)
            throws Exception {
        Method method =
                QueryRecommendProcessor.class.getDeclaredMethod("doProcess", ParseContext.class);
        method.setAccessible(true);
        method.invoke(processor, parseContext);
    }
}
